...
def arepo_m_gen(fname,field_add):
    ...
    if cfg.par.otf_extinction:
        #each Voronoi cell is generated by one dust particle, so the
        #mass-conserving cell density is that particle's dust mass over
        #the volume the tessellation assigned to it.
        dust_particle_mass = reg["PartType3", "Masses"].in_units('g').value
        cell_volume = m.grid.volumes
        valid = cell_volume > 0
        dustdens = np.zeros_like(dust_particle_mass)
        dustdens[valid] = dust_particle_mass[valid] / cell_volume[valid]
        if (~valid).any():
            print('[arepo_tributary]: %d Voronoi cells have invalid volumes; '
                  'their dust is omitted (%.3e Msun of %.3e)'
                  % ((~valid).sum(),
                     dust_particle_mass[~valid].sum() / 1.989e33,
                     dust_particle_mass.sum() / 1.989e33))

...
def allstars_sed_gen(stars_list,diskstars_list,bulgestars_list):
    ...
    if cfg.par.COSMOFLAG == False:
        ...
        sp = fsps.StellarPopulation(tage = cfg.par.bulge_stars_age,imf_type=cfg.par.imf_type,pagb = cfg.par.pagb,sfh=0,zmet=cfg.par.bulge_stars_metals,add_neb_emission = cfg.par.add_neb_emission)
...
def newstars_gen(stars_list):
    ...
    sp = fsps.StellarPopulation(tage=stars_list[0].age,imf_type=cfg.par.imf_type,pagb = cfg.par.pagb,sfh=0,zmet=stars_list[0].fsps_zmet,add_neb_emission = cfg.par.add_neb_emission)
    spec = sp.get_spectrum(tage=stars_list[0].age,zmet=stars_list[0].fsps_zmet)
    nu = 1.e8*constants.c.cgs.value/spec[0]
    ...
    for i in range(len(stars_list)):
        ...
        if cfg.par.CF_on == True:
            sp = fsps.StellarPopulation(tage=stars_list[i].age,imf_type=cfg.par.imf_type,pagb = cfg.par.pagb,sfh=0,zmet=stars_list[i].fsps_zmet,dust_type=0,dust1=1,dust2=0,dust_tesc=tesc_age,add_neb_emission = cfg.par.add_neb_emission)
        else:
            sp = fsps.StellarPopulation(tage=stars_list[i].age,imf_type=cfg.par.imf_type,pagb = cfg.par.pagb,sfh=0,zmet=stars_list[i].fsps_zmet,add_neb_emission = cfg.par.add_neb_emission)
